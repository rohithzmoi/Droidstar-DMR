...
- (void)setPreferredInputDevice; // New method for setting the preferred input device
@end
...
- (void)setupAVAudioSession {
    @try {
        ...
        // Set preferred input device (if any)
...
- (void)setPreferredInputDevice {
    ...
    // Loop through available inputs and set the preferred one
    for (AVAudioSessionPortDescription *input in availableInputs) {
        NSLog(@"Available input: %@ - %@", input.portName, input.portType);
        ...
        if ([input.portType isEqualToString:AVAudioSessionPortBluetoothHFP] ||
            ...
            // Set Bluetooth device as preferred input
            BOOL success = [session setPreferredInput:input error:&error];
            if (success) {
                NSLog(@"Preferred input set to Bluetooth: %@", input.portName);
            } else {
                NSLog(@"Error setting preferred input to Bluetooth: %@, code: %ld", error.localizedDescription, (long)error.code);
            }
            return;
        } else if ([input.portType isEqualToString:AVAudioSessionPortHeadsetMic]) {
            // Fallback to wired headset mic
            BOOL success = [session setPreferredInput:input error:&error];
            if (success) {
                NSLog(@"Preferred input set to Headset Mic: %@", input.portName);
            } else {
                NSLog(@"Error setting preferred input to Headset Mic: %@, code: %ld", error.localizedDescription, (long)error.code);
            }
            return;
        }
    }

    NSLog(@"No preferred input device found.");
}
...
- (void)handleAudioRouteChange:(NSNotification *)notification {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (reason == AVAudioSessionRouteChangeReasonOldDeviceUnavailable ||
            ...
            NSLog(@"Audio route change detected, setting preferred input...");
            [self setupAVAudioSession];
        }
...
extern "C" void setPreferredInputDevice() {
    AudioSessionManager *audioManager = [[AudioSessionManager alloc] init];
    [audioManager setPreferredInputDevice];
}
